...
#' admm algorithm of joint graphical lasso
#'
#' @param Y
#' @param penalty
#' @param lambda1
...
#' @param tol
#' @param warm
#' @param return.whole.theta
#' @param screening
#' @param truncate
#' @export
#' @returns a network list with length equal to that of Y
...
JGL_cov <-
  function(SS,n,penalty="fused",lambda1,lambda2,rho=1,weights="sample.size",penalize.diagonal=FALSE,maxiter=500,tol=1e-5,warm=NULL,return.whole.theta=FALSE, screening="fast",truncate=1e-5)
  {
    ## initialize:
    p = dim(SS[[1]])[2]
    K = length(SS)
    ...
    # assign feature names if none exist:
    if(length(dimnames(SS[[1]])[[2]])==0)
    {
      for(k in 1:K)
      {
        dimnames(SS[[k]])[[2]]=paste("V",1:p,sep="")
      }
    }
  ...
  connected = rep(TRUE,p)
    ...
    for(k in 1:K)
    {
      ...
      if(sum(connected)>0)
      {
        dimnames(theta[[k]])[[1]]=dimnames(theta[[k]])[[2]]=dimnames(SS[[k]])[[2]][connected]
      }
    }
    ...
    # data:
    Su = list()
    for(k in 1:K){Su[[k]] = SS[[k]][,unconnected]}
    # penalty vectors:
    # note: for admm.iters.unconnected, we use the penalize.diagonal argument before calling the function.  for admm.iters, we use it IN the function.
    if(length(lambda1)==1) { lam1.unconnected = lambda1 }
    if(length(lambda1)>1) { lam1.unconnected = diag(lambda1)[unconnected] }
    ...
    if(sum(unconnected)>0)
    {
      theta.unconnected = admm.iters.unconnected_cov(Su,lambda1=lam1.unconnected,lambda2=lam2.unconnected,penalty=penalty,rho=rho,weights=weights,maxiter=maxiter,tol=tol)$Z
      for(k in 1:K) { names(theta.unconnected[[k]])=dimnames(SS[[k]])[[2]][!connected] }
    }
    if(sum(unconnected)==0) {theta.unconnected = NULL}
    ...
    if(length(blocklist)>0){
      for(i in 1:length(blocklist)){
        ...
        lam2.bl = penalty.as.matrix(lam2.bl,dim(Sbl[[1]])[2],penalize.diagonal=TRUE)
        ...
        # run JGL on the block:
        Thetabl = admm.iters_cov(Sbl,lam1.bl,lam2.bl,penalty=penalty,rho=rho,weights=weights,penalize.diagonal=TRUE,maxiter=maxiter,tol=tol,warm=warm.bl)
        # update Theta with Thetabl's results:
        for(k in 1:K) {theta[[k]][connected.index[bl],connected.index[bl]] = Thetabl$Z[[k]]}
      }}
