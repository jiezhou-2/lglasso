...
```{r,warning=FALSE}
aa=lglasso(data=ddata,lambda = 0.01)
...
```{r,warning=FALSE}
aa=lglasso(data=ddata,lambda = 0.01,random=TRUE)
...
```{r,warning=FALSE}
dd=Simulate(type="longihomo",n=n,p=p,m1=m1,m2=m2,tau=c(2,1),tt=20)
...
aa=lglasso(data=ddata,lambda = c(0.1,0.1),group = group)
estimates=lapply(aa$wi,function(ll){ifelse(abs(ll)>10^(-3),1,0)})
...
aa=lglasso(data=ddata,lambda = c(0.1,0.1),random=T,group = group)
estimates=lapply(aa$wi,function(ll){ifelse(abs(ll)>10^(-3),1,0)})
