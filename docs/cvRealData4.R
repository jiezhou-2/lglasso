...
postdata[,-c(1,2)]=log10(1+postdata[,-c(1,2)])
...
lambda1=exp(seq(-5,-1,length=2))
lambda2=lambda1
lambda=expand.grid(lambda1,lambda2)

### two-stage model (full data)

lambda1=exp(seq(-10,0,length=10))
lambda2=exp(seq(-5,0,length=10))
lambda=expand.grid(lambda1,lambda2)
aa1=CVlglasso(data=fulldata,lambda = lambda,K=2,trace = FALSE,random = TRUE,group = group)
saveRDS(aa1,file="heter2.rds")
